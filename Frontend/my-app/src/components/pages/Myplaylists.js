...
export default function Myplaylists() {
  ...
  function rem(id) {
    ...
    axios
      .post('http://localhost:8080/api/playlists/remove-song', data)
      .then(() => {
  ...
  function createPlaylist() {
    ...
    axios
      .post('http://localhost:8080/api/playlists', data)
      .then((response) => {
      ...
      if (!songs || songs.length === 0) {
        content = (
          <div className="text-center mt-10">
            ...
            <Link to="/home">
              <button className="bg-green-600 text-white place-content-center px-[10px] py-[4px] my-[5px]">
                Add Songs
              </button>
            </Link>
          </div>
